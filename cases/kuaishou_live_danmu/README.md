...
> Python：3.10+（需 `pip install curl_cffi protobuf`）
...
`kuaishou_live_danmu.py`：Python（curl_cffi Chrome TLS + 手写 protobuf wire format），
实现了签名生成 + websocketinfo + WS 进房 + 弹幕解码的完整协议栈。

```bash
python kuaishou_live_danmu.py --room 3xdw2ep3jh6k9y4 --count 20
# --lsid 留空自动解析；--cookie 留空则从已连接调试浏览器采集并缓存
```
...
1. **`__NS_hxfalcon` 纯算法可本地复现（result:1 验证通过）**
   ...
   - 纯 Python 实现见 `kuaishou_hxfalcon.py`，关键函数：`generate_hxfalcon` / `build_sign_input` / `sign_live_api_url` / `build_sign_headers`。
